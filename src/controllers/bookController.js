...
    // Apply availability status filter - Optimized by availabilityStatus index
...
          break;
        case "accession":
          // Accession index is unique, providing fast lookup
...
          break;
        default:
          // Search across multiple fields - benefits from text index but specific fields like accession_number benefit more from dedicated indexes
          filter.$or = [
...
      .sort({ accession_number: 1 }); // Sort by accession number is fast due to unique index
...
    // --- NEW CHECK: Prevent setting status to lost/write-off if currently issued ---
    if (availabilityStatus === 'lost' || availabilityStatus === 'write-off') {
...
      }
      ...
      // If the book is currently marked as 'issued', prevent administrative change.
      if (currentBook.availabilityStatus === 'issued') {
        return res.status(400).json({
          success: false,
          message: "Cannot mark a book as LOST or WRITE-OFF while it is ISSUED. It must be returned or managed via the transactions/fines module first."
        });
      }
    }
    // --- END NEW CHECK ---
...
    // Check if book is currently issued (using the explicit index on issuedBooks)
...
      return res.status(400).json({
        success: false,
        message: "Cannot delete book. It is currently issued to one or more members. All books must be returned first.",
...
  try {
    // These queries now leverage the new 'availabilityStatus' index.
